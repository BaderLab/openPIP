	 		...
	 		tissueArray.forEach(function(tissue){
	 			if(TissueExpressionParameterArray[tissue]){
		        	...
		        	if( 5.0 > tissueExpressionArray[tissue] || typeof tissueExpressionArray[tissue] == "undefined"){
		 				...
		 				console.log(tissueExpressionArray[tissue]);
		        		removeProtein = true;		
		        	} 
	 			}
	        });
...
function updateEnrichedTerms(){
	...
	$.getJSON(url, function(JsonData) {
		...
		$("#MF_table").empty();
	 	...
	 	$("#hp_table").empty();
	 	if(typeof JsonData.hp !== 'undefined' && JsonData.hp.length > 0 ){	

		 	JsonData.hp.forEach(function(hp_row, i) {
				$("#hp_table").append("<tr class='enrichment_row' data='" + hp_row['list'] + "' term='" + hp_row['GO_term'] + "'><td><a href='http://compbio.charite.de/hpoweb/showterm?id="+ hp_row['GO_term_code'] + "' target='_blank'>"  + hp_row['GO_term_code'] + "</a></td><td>" + hp_row['GO_term'] + "</td><td></td></tr><div class='hidden'></div>");	 
		 	});
	 	}else{
	 	
	 		$("#hp_table").append("<tr><td colspan='3'>No Phenotypes</td></tr>");		
	 	}
	 	$("#hp_table").trigger('footable_redraw');
	 	
	 	$("#omi_table").empty();
	 	if(typeof JsonData.omi !== 'undefined' && JsonData.omi.length > 0 ){	

		 	JsonData.omi.forEach(function(omi_row, i) {
				$("#omi_table").append("<tr class='enrichment_row' data='" + omi_row['list'] + "' term='" + omi_row['GO_term'] + "'><td><a href='https://www.omim.org/entry/"+  omi_row['GO_term_code'].replace("OMIM:", "") + "' target='_blank'>"  + omi_row['GO_term_code'] + "</a></td><td>" + omi_row['GO_term'] + "</td><td></td></tr><div class='hidden'></div>");	 
		 	});
	 	}else{
	 	
	 		$("#omi_table").append("<tr><td colspan='3'>No OMIM Terms</td></tr>");		
	 	}
	 	$("#omi_table").trigger('footable_redraw');
...
function updateExternalLinks(){
	...
	$("#gene_mania_link_li").html('<a href="' + geneManiaUrl + '" target="_blank">GeneMania</a>');
	$("#string_link_li").html('<a href="' + stringUrl + '" target="_blank">STRING</a>');
	$("#reactome_link_li").html('<a id="reactome_link" style="cursor: pointer;" data="' + reactomeUrl + '">Reactome</a>');
	$("#pathway_commons_link_li").html('<a href="' + pathwayCommonsUrl + '" target="_blank">Pathway Commons</a>');
	$("#david_link_li").html('<a href="' + davidUrl + '" target="_blank">DAVID</a>');
	$("#gprofiler_link_li").html('<a href="' + gProfiler + '" target="_blank">gProfiler</a>');
	$("#cbioportal_link_li").html('<a href="' + cBioPortal + '" target="_blank">cBioPortal</a>');
	$("#intact_link_li").html('<a href="' + intAct + '" target="_blank">IntAct (Query)</a>');
	$("#complex_portal_link_li").html('<a href="' + complexPortal + '" target="_blank">Complex Portal</a>');
