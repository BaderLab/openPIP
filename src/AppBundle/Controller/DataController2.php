...
class DataController extends Controller
{
	...
	public function handleData($data_form){
		...
		$handle = fopen('D:\\test\\LitBM-17.psi', 'r');
		$file_row = 0;
		while ($file_data = fgetcsv($handle, 0, "\t")){
			$file_row++;
			if($file_row < 3){ continue; }
			try{
				...
				if(self::isNewInteraction($interactor_A_id, $interactor_B_id) == true){
					$protein_A = self::proteinHandler($interactor_A_id);
					$protein_B = null;
					if($interactor_A_id == $interactor_B_id){
						$protein_B = $protein_A;
					}else{
						$protein_B = self::proteinHandler($interactor_B_id);
					}
	...
	public function handleData($data_form){
		...
		while ($file_data = fgetcsv($handle, 0, "\t")){
			...
			try{
				...
				$interaction = self::getInteraction($interactor_A_id, $interactor_B_id, $alt_interactor_A_id, $alt_interactor_B_id);
				...
				$dataset = self::getDatasetByReference('HI-III');
	...
	public function getInteraction($interactor_A_id, $interactor_B_id){
		$interaction = null;
	...
	public function getExperiment($interactor_A_alias, $interactor_B_alias){
		$interaction = null;
		...
		if(self::isNewExperiment($interactor_A_alias, $interactor_B_alias) == true){
			...
			$interaction = self::getExperimentByOrfIds($interactor_A_alias, $interactor_B_alias);
		}
	...
	/*
	public function getDumbForm($data_form){
		...
		if ($data_form->isSubmitted()) {
			...
			/*
			 ...
			 $fileName = $file->getClientOriginalName();ccccccccccccccccc
			 $file->move(
			...
			while ($file_data = fgetcsv($handle, 0, "\t"))
			{
				...
				try{
					...
					list ($interactor_A_id, $interactor_B_id) = $file_data;
					if(self::isNewInteraction($interactor_A_id, $interactor_B_id) == true){
	...
	*/
	...
	public function proteinHandler($interactor_id){



		$protein = self::createProteinFromData($interactor_id);
	...
	public function createExperimentFromData($interactor_A_id, $interactor_B_id, $alt_interactor_A_id, $alt_interactor_B_id, $interactor_A_alias, $interactor_B_alias, $annotation_interactor_A, $annotation_interactor_B, $interaction_annotation){
		...
		$assay_version = getAssayVersion($annotation_interactor_A, $annotation_interactor_B);
		
		$protein_A = self::getProtein($interactor_A_id,  $alt_interactor_A_id);
		$protein_B = self::getProtein($interactor_B_id,  $alt_interactor_B_id);
	...
	public function createInteractionFromData(){
	...
	*/
	...
	public function createProteinFromData($interactor_id){
...
*/
	...
	public function getIdentifierNamingConventionAndId($interactor_id){
		...
		if($interactor_id == '-'){
			...
			$identifier_identifier_interactor_id = $matchesA[1];
	...
	public function isNewInteraction($interactor_A_id, $interactor_B_id){
		...
		$identifier_naming_convention_and_id_A = self::getIdentifierNamingConventionAndId($interactor_A_id, $alt_interactor_A_id);
		$naming_convention_A = $identifier_naming_convention_and_id_A[0];
		$identifier_A_identifier_interactor_id = $identifier_naming_convention_and_id_A[1];
		
		$identifier_naming_convention_and_id_B = self::getIdentifierNamingConventionAndId($interactor_B_id, $alt_interactor_B_id);
		$naming_convention_B = $identifier_naming_convention_and_id_B[0];
		$identifier_B_identifier_interactor_id = $identifier_naming_convention_and_id_B[1];
		
		$protein_A_id = self::getProteinId($identifier_A_identifier_interactor_id, $naming_convention_A);
		$protein_B_id = self::getProteinId($identifier_B_identifier_interactor_id, $naming_convention_B);
